...
!function (e, t) {
    ...
    var t = function (e, t) {
        ...
        if (Array.prototype.map && e.map === Array.prototype.map)
            ...
            for (o = 0; s > o; o++)
                callback_params = n, callback_params.splice(0, 0, e[o]), r.push(t.apply(this, callback_params));
        return r
    }, n = function (e) {
        var t, o = [];
        ...
        for (o = 0; o < e.length; o++)
            e[o]instanceof google.maps.LatLng || (e[o] = e[o].length > 0 && "object" == typeof e[o][0] ? s(e[o], t) : r(e[o], t));
        return e
    }, i = function (e, t) {
        var o, e = e.replace("#", "");
        ...
        var e = document, o = function (n) {
            ...
            if (this.el = "string" == typeof n.el || "string" == typeof n.div ? i(c, n.context) : c, "undefined" == typeof this.el || null === this.el)
                throw"No element defined.";
            for (window.context_menu = window.context_menu || {}, window.context_menu[s.el.id] = {}, this.controls = [], this.overlays = [], this.layers = [], this.singleLayers = {}, this.markers = [], this.polylines = [], this.routes = [], this.polygons = [], this.infoWindow = null, this.overlay_el = null, this.zoom = n.zoom, this.registered_events = {}, this.el.style.width = n.width || this.el.scrollWidth || this.el.offsetWidth, this.el.style.height = n.height || this.el.scrollHeight || this.el.offsetHeight, google.maps.visualRefresh = n.enableNewStyle, r = 0; r < g.length; r++)
                delete n[g[r]];
            for (1 != n.disableDefaultUI && (x = t(x, C)), M = t(x, n), r = 0; r < l.length; r++)
                delete M[l[r]];
            ...
            google.maps.event.addListener(this.map, "rightclick", function (e) {
                ...
                for (var t = e.length, o = new google.maps.LatLngBounds, n = 0; t > n; n++)
                    o.extend(e[n]);
    ...
    l.prototype.createControl = function (e) {
        ...
        for (var t = null, o = 0; o < this.controls.length; o++)
            this.controls[o] == e && (t = this.controls[o].position, this.controls.splice(o, 1));
        if (t)
        ...
        if (l.fences = r, e.infoWindow) {
            ...
            for (var p = ["closeclick", "content_changed", "domready", "position_changed", "zindex_changed"], g = 0; g < p.length; g++)
                !function (t, o) {
        ...
        for (var c = ["animation_changed", "clickable_changed", "cursor_changed", "draggable_changed", "flat_changed", "icon_changed", "position_changed", "shadow_changed", "shape_changed", "title_changed", "visible_changed", "zindex_changed"], h = ["dblclick", "drag", "dragend", "dragstart", "mousedown", "mouseout", "mouseover", "mouseup"], g = 0; g < c.length; g++)
            !function (t, o) {
        ...
        if ("undefined" == typeof e) {
            ...
            for (var o = 0; o < e.length; o++)
                this.markers.indexOf(e[o]) > -1 && this.markers[o].setMap(null);
            for (var o = 0; o < this.markers.length; o++)
                null != this.markers[o].getMap() && t.push(this.markers[o]);
            this.markers = t
        }
        ...
        for (var i = new google.maps.Polyline(s), a = ["click", "dblclick", "mousedown", "mousemove", "mouseout", "mouseover", "mouseup", "rightclick"], p = 0; p < a.length; p++)
            !function (t, o) {
        ...
        e.bounds = o;
        for (var n = new google.maps.Rectangle(e), r = ["click", "dblclick", "mousedown", "mousemove", "mouseout", "mouseover", "mouseup", "rightclick"], s = 0; s < r.length; s++)
            !function (t, o) {
                e[o] && google.maps.event.addListener(t, o, function (t) {
                    e[o].apply(this, [t])
                })
        ...
        for (var i = new google.maps.Polygon(e), a = ["click", "dblclick", "mousedown", "mousemove", "mouseout", "mouseover", "mouseup", "rightclick"], p = 0; p < a.length; p++)
            !function (t, o) {
                e[o] && google.maps.event.addListener(t, o, function (t) {
                    e[o].apply(this, [t])
                })
    ...
    return l.prototype.getRoutes = function (e) {
        ...
        s.route(n, function (t, o) {
            if (o === google.maps.DirectionsStatus.OK) {
                for (var n in t.routes)
                    t.routes.hasOwnProperty(n) && r.routes.push(t.routes[n]);
        ...
        if (e.path) {
            ...
            delete e.path, delete e.samples, i.getElevationForLocations(e, function (e, t) {
                r && "function" == typeof r && r(e, t)
            })
        ...
        if (e.origin && e.destination)
            this.getRoutes({origin: e.origin, destination: e.destination, travelMode: e.travelMode, waypoints: e.waypoints, unitSystem: e.unitSystem, error: e.error, callback: function (t) {
                    if (t.length > 0 && e.start && e.start(t[t.length - 1]), t.length > 0 && e.step) {
                        var o = t[t.length - 1];
                        if (o.legs.length > 0)
                            for (var n, r = o.legs[0].steps, s = 0; n = r[s]; s++)
                                n.step_number = s, e.step(n, o.legs[0].steps.length - 1)
                    }
        ...
        var t = this;
        if (e.origin && e.destination)
            this.getRoutes({origin: e.origin, destination: e.destination, travelMode: e.travelMode, waypoints: e.waypoints, error: e.error, callback: function (o) {
                    if (o.length > 0 && e.start && e.start(o[o.length - 1]), o.length > 0 && e.step) {
                        var n = o[o.length - 1];
                        if (n.legs.length > 0)
                            for (var r, s = n.legs[0].steps, i = 0; r = s[i]; i++)
                                r.step_number = i, t.drawPolyline({path: r.path, strokeColor: e.strokeColor, strokeOpacity: e.strokeOpacity, strokeWeight: e.strokeWeight}), e.step(r, n.legs[0].steps.length - 1)
                    }
                    o.length > 0 && e.end && e.end(o[o.length - 1])
                }});
        else if (e.route && e.route.legs.length > 0)
            for (var o, n = e.route.legs[0].steps, r = 0; o = n[r]; r++)
                o.step_number = r, t.drawPolyline({path: o.path, strokeColor: e.strokeColor, strokeOpacity: e.strokeOpacity, strokeWeight: e.strokeWeight}), e.step(o)
    }, l.Route = function (e) {
        ...
        if (t.size = e.size || [this.el.clientWidth, this.el.clientHeight], t.lat = this.getCenter().lat(), t.lng = this.getCenter().lng(), this.markers.length > 0) {
            t.markers = [];
            for (var o = 0; o < this.markers.length; o++)
                t.markers.push({lat: this.markers[o].getPosition().lat(), lng: this.markers[o].getPosition().lng()})
        }
        ...
        if (delete e.polyline, e.center)
            ...
            n.push(["center=", e.lat, ",", e.lng].join("")), delete e.lat, delete e.lng;
        else if (e.visible) {
        ...
        for (var c in e)
            e.hasOwnProperty(c) && n.push(c + "=" + e[c]);
        if (s)
            for (var h, u, d = 0; o = s[d]; d++) {
                ...
                for (var c in o)
                    o.hasOwnProperty(c) && h.push(c + ":" + o[c]);
        ...
        for (var n = ["closeclick", "links_changed", "pano_changed", "position_changed", "pov_changed", "resize", "visible_changed"], r = t({visible: !0}, e), s = 0; s < n.length; s++)
            delete r[n[s]];
        ...
        if (-1 == l.custom_events.indexOf(e))
            return t instanceof l && (t = t.map), google.maps.event.addListener(t, e, o);
        ...
        if (0 === o)
            return-1;
        var n = 0;
        if (arguments.length > 1 && (n = Number(arguments[1]), n != n ? n = 0 : 0 != n && 1 / 0 != n && n != -1 / 0 && (n = (n > 0 || -1) * Math.floor(Math.abs(n)))), n >= o)
            return-1;
